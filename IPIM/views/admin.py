...
def admin():
  ...
  if logged:
    ...
    ######## get maxsteps ######### [label array<str>, count array<int>]
    maxsteps = get_maxstep_array(ga_analytics)
...
# mark_analyze
@app.route('/admin/mark_tag/', methods=['POST', 'GET'])
def mark_tag():
    # print "in mark_tag"
    if request.method == 'POST':
        cid = request.form['cid']
        ...
        cur.close()
        return render_template("/admin/mark_tag.html", cid=cid, tagname=tagname)
    return render_template("/admin/mark_tag.html")
